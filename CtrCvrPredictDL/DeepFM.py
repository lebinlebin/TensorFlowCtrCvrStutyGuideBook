...
def model_fn(features, labels, mode, params):
   ...
   #------build feaure-------
    ...
    with tf.variable_scope("Deep-part"):
        ...
        for i in range(1):
            print(">>>>>>>>>>>>>>>>>>>>>>>>>> len(list(layers))                 ", i)
            #if FLAGS.batch_norm:
            #    deep_inputs = batch_norm_layer(deep_inputs, train_phase=train_phase, scope_bn='bn_%d' %i)
                #normalizer_params.update({'scope': 'bn_%d' %i})
            deep_inputs = tf.contrib.layers.fully_connected(inputs=deep_inputs, num_outputs=list(layers)[i], \
            ...
            if mode == tf.estimator.ModeKeys.TRAIN:
                deep_inputs = tf.nn.dropout(deep_inputs, keep_prob=list(dropout)[i])                              #Apply Dropout after all BN layers and set dropout=0.8(drop_ratio=0.2)
