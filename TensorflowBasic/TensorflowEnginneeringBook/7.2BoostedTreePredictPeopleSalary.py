...
LOSS_PREFIX = {'wide': 'linear/', 'deep': 'dnn/','BoostedTrees':'BoostedTrees/'}#定义模型的前缀
...
def build_model_columns():
  ...
  boostedtrees_columns = [age_buckets,
    tf.feature_column.bucketized_column(education_num, boundaries=[4, 5, 7, 9, 10, 11, 12, 13, 14, 15]),
    tf.feature_column.bucketized_column(capital_gain, boundaries=[1000, 5000, 10000, 20000, 40000,50000]),
    tf.feature_column.bucketized_column(capital_loss, boundaries=[100, 1000, 2000, 3000, 4000]),
    tf.feature_column.bucketized_column(hours_per_week, boundaries=[7, 14, 21, 28, 35, 42, 47, 56, 63, 70,77,90]),
    tf.feature_column.indicator_column(workclass),              #将workclass列的稀疏矩阵转成0ne_hot编码
    tf.feature_column.indicator_column(education),
    tf.feature_column.indicator_column(marital_status),
    tf.feature_column.indicator_column(relationship),
  ...
  return wide_columns, deep_columns,boostedtrees_columns


def build_estimator(model_dir, model_type,warm_start_from=None):
  """按照指定的模型生成估算器对象."""
  wide_columns, deep_columns ,boostedtrees_columns= build_model_columns()
  ...
  if model_type == 'wide':                                      #生成带有wide模型的估算器对象
    ...
    return tf.estimator.DNNClassifier(
        ...
        config=run_config)
  elif model_type=='BoostedTrees':
    return tf.estimator.BoostedTreesClassifier(
        model_dir=model_dir,
        feature_columns=boostedtrees_columns,
...
def input_fn(data_file, num_epochs, shuffle, batch_size):       #定义估算器输入函数
  """估算器的输入函数."""
  assert tf.gfile.Exists(data_file), (                          #用断言语句判断样本文件是否存在
...
def export_model(model, model_type, export_dir):                #定义函数export_model ，用于导出模型
  ...
  参数:
    ...
    export_dir: 导出模型的路径.
  """
  wide_columns, deep_columns ,boostedtrees_columns= build_model_columns()        #获得列张量
  if model_type == 'wide':
    columns = wide_columns
  elif model_type == 'deep'  :
    columns = deep_columns
  elif 'BoostedTrees'==model_type:
    columns = boostedtrees_columns
  else:
...
def trainmain(argv):
  ...
  for n in range(flags.train_epochs ): #将总迭代数，按照epochs_between_evals分段。并循环对每段进行训练
    if flags.model_type == 'BoostedTrees':
        model.train(input_fn=train_input_fn)
    else:
        model.train(input_fn=train_input_fn, hooks=[train_hook])         #调用估算器的train方法进行训练
...
if __name__ == '__main__':                  #当运行文件时，模块名字__name__就会为__main__
  tf.logging.set_verbosity(tf.logging.ERROR) #设置log级别为INFO，如果想要显示的信息少点，可以设置成 WARN
